...
```{r}
summary(darwin$cross)
summary(darwin$self)
ggplot(darwin) +
  geom_histogram(aes(x = cross), bins = 5)
ggplot(darwin) +
  geom_histogram(aes(x = self), bins = 5)
ggplot(darwin) +
  geom_point(aes(x = cross, y = self)) +
  geom_abline(slope = 1, intercept = 0) +
  theme_bw()
```

**We can create separate numerical summaries for cross- and self-fertilized plants using something like `summary()`**

**We saw that it is hard to graph data like this in ways that we are used t because we can't map which type of fertilization a plant was to an aesthetic element if we have two variables for that. In our summary and in our graphs we did get to see that cross fertilized plants tend to grow taller than self fertilized in Darwin's data**
...
**The scatter plot at the end was the only graph that linked each pair of plants in the same pot to one another. Darwin setting up the study this way meant each pair of plants had the same growth conditions (sun, fertilizer, soil moisture, etc.)**
...
```{r}
xbar_d <- mean(darwin$diff)
```


**The sample mean of the differences. This value is `r round(xbar_d, 2)` inches**
...
```{r}
get_ci(darwin_boot, type = "se", point_estimate = xbar_d, level = 0.9)
```
...
**If our interval is entirely positive, we are 90% confident our first group (cross-fertilized) has a higher population mean. If they were all negative, we would be 90% confident our second group (self-fertilized) has a higher population mean.**
...
**We are 90% confident that the true population mean height is between 0.7 and 4.5 inches taller for cross-fertilized plants than for self-fertilized plants.**
...
```{r}
library(Sleuth3)
twins <- mutate(case0202, diff = Unaffected - Affected)
```

First I loaded the library containing the data and created a variable that calculates the difference in left hippocampus volume for each pair of twins (by subtracting the value for the affected twin from the value for the unaffected twin). Next I will create a bootstrap distribution.
...
twins_boot <- twins %>%
  ...
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "mean")
visualize(twins_boot)
```

The bootstrap distribution contains almost entirely positive numbers. It is centerered around 0.2 and looks roughly bell-shaped. The values go up to around 0.4. Next I will calculate a 95% confidence interval for the difference in means using a percentile method.

```{r}
get_ci(twins_boot, type = "percentile", level = 0.95)
```

The confidence interval is (0.098, 0.321). Since I calculated my `diff` variable as the unaffected twin's value minus the affected twin's value, my interpretation would be:

> I am 95% confident the population mean left hippocampus volume for those unaffected by schizophrenia is between 0.098 and 0.321 $cm^3$ bigger than it is for those affected by schizophrenia.